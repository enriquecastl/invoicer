...
const invoiceData = flags.invoiceData || config.defaultInvoiceEntryFile;
...
try {
  const invoice = new CreateInvoiceService(db, config).execute(
    JSON.parse(Deno.readTextFileSync(invoiceData as string))
  );

  console.log(
    `Invoice with NCF ${displayInvoiceNCFCode(
      invoice.ncf
