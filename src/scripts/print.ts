...
const dest = flags.dest || config.printDest;
...
if (!dest) {
  console.log(
    "Please, provide a destination path where to save the invoice document"
  );
  Deno.exit(1);
}
...
try {
  const { invoiceDocument, ncfFormatted } = await new PrintInvoiceService(
    db,
    config
  ).execute(ncf);
  ...
  console.log(
    `Saving invoice document with NCF ${ncfFormatted} in path ${fullPathPdf}`
  );
